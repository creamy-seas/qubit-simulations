...
EC = 1/(6450*5.7E-15)/1E9; %EC=(2e)^2/CJ in GHz
...
alphaJ = 1.023; alphaJ = 1.023;
Nch = 9; % Number of5 charge states used for calculations. Generally (EC*((Nch-1)/2)^2)/EJ > 1
...
for n1 = 1:Nch-1 for n2 = 1:Nch-1 for n3 = 1:Nch
   ...
   dH12(j,k)=1; dH21(k,j)=1; end; end; end;
for n1 = 1:Nch for n2 = 1:Nch-1 for n3 = 1:Nch-1
   j0=vq*[n1-1;n2-1;n3-1]+1; j=j0+vq*[0;0;1]; k=j0+vq*[0;1;0]; H(j,k)=-0.5*EJ; H(k,j)=-0.5*EJ; dH32(j,k)=1; dH23(k,j)=1; end; end; end;
for n3 = 1:Nch for n2 = 1:Nch-1 for n1 = 1:Nch
   j0=vq*[n1-1;n2-1;n3-1]+1; j=j0; k=j0+vq*[0;1;0]; H(j,k)=-0.5*EJa; H(k,j)=-0.5*EJa; dH02(j,k)=1; dH20p(j,k)=exp(-i*pi); dH20(k,j)=1; end; end; end;
...
for n1 = 1:Nch for n2 = 1:Nch for n3 = 1:Nch
   qv = [q(n1) q(n2) q(n3)]; j=vq*[n1-1 n2-1 n3-1]'+1; H(j,j)=0.5*qv*Cinv*qv';
   qv1 = [0 1 0]; dH1(j,j)=qv1*Cinv*qv';
end;end;end;
...
for nfx = 1:nloop
  for nfy = 1:nloop
      ...
      for n1 = 1:Nch-1 for n2 = 1:Nch for n3 = 1:Nch
        ...
        H(j,k)=-0.5*EJ*exp(-i*2*pi*f); 
        H(k,j)=-0.5*EJ*exp(i*2*pi*f);
        
        
        dH01(j,k)=exp(i*2*pi*f); dH10(k,j)=exp(-i*2*pi*f);
      end;end;end;
      for n1 = 1:Nch for n2 = 1:Nch for n3 = 1:Nch-1
        j0=vq*[n1-1;n2-1;n3-1]+1; j=j0; k=j0+vq*[0;0;1]; H(j,k)=-0.5*EJ*exp(i*2*pi*f1); H(k,j)=-0.5*EJ*exp(-i*2*pi*f1);
...
%       ds=real(svecD(1,:)); svec = svecD(2:size(vd(:,1))+1,:);
      ...
      ds1(nfx,nfy) = ds(1);
      ds2(nfx,nfy) = ds(2); ds3(nfx,nfy) = ds(3); ds4(nfx,nfy) = ds(4); ds5(nfx,nfy) = ds(5);
