...
class TwinQubitPlotter:
    ...
    def plot_dipole_matrix_elements(self, mpl_axes: Axes):
        logging.info("Plotting transition matrix elemnents")

        mpl_axes.plot(
            self.flux_list, self.twin_qubit_simulator.simulations["d32"], color="C5"
        )
        mpl_axes.plot(
            self.flux_list, self.twin_qubit_simulator.simulations["d21"], color="C6"
        )
        mpl_axes.plot(
            self.flux_list, self.twin_qubit_simulator.simulations["d13"], color="C7"
        )
    ...
    def plot_dipole_moment_voltage(self, mpl_axes: Axes):
        ...
        mpl_axes.set_xlabel("Normalised Magnetic Flux ($\Phi/\Phi_0$)")
        mpl_axes.set_ylabel(r"Matrix elements ($\mu$V)")
