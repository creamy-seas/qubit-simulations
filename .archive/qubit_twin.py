...
from qubit_master import qubit_master
import honkler
import time
# plt.style.use('ilya_plot')


class twin(qubit_master):
    """
    ...
    def __init__(
        ...
        [bool] cut_interaction:         how many interactions between the higher level charge states to ignore
        ...
        """
        qubit_master.__init__(self, plot_or_not, message_or_not)

        #######################################################################
        #                            new cross-term                            #
        #######################################################################
        self.cut_interaction = cut_interaction
    ...
    def prepare_hamiltonian_stage1(self):
        ...
        for x in range(0, self.states_total_number):
            ...
            if (self.state_numerical_distribution[0] < (self.states_per_island - 1 - self.cut_interaction)):
                # cos(phi_1)
                y = self.convert_numerical_state_to_index(
                ...
                if (self.state_numerical_distribution[1] > 0 + self.cut_interaction):
                    y = self.convert_numerical_state_to_index(
                        self.state_numerical_distribution + [1, -1, 0])
                    self.op_H_phi_row.extend([x, y])
                    self.op_H_phi_col.extend([y, x])

            if (self.state_numerical_distribution[1] < (self.states_per_island - 1 - self.cut_interaction)):
                # alpha * cos(phi_2)
                y = self.convert_numerical_state_to_index(
                    self.state_numerical_distribution + [0, 1, 0])
            ...
            if (self.state_numerical_distribution[2] < (self.states_per_island - 1 - self.cut_interaction)):
                # cos(phi_3)
                y = self.convert_numerical_state_to_index(
    ...
    def simulate(self, simulate_expectations):
        ...
        for ext_flux_number in range(0, len(self.flux_list)):
            ...
            if(simulate_voltage):
                ...
                temp_dipole_moment_voltage_beta = matrix01 / self.const_Phi0 / self.spectrum_simulation_12[ext_flux_number]

                self.dipole_moment_voltage['01'].append([matrix01.real,
                                                         matrix01.imag])
                self.dipole_moment_voltage['02'].append([matrix02.real,
                                                         matrix02.imag])
                self.dipole_moment_voltage['12'].append([matrix12.real,
                                                         matrix12.imag])
...
if (__name__ == "__main__"):
    ...
    for i in range(0, (no_states - 3) // 2):
        print(i)
        my_run = twin(alpha, assymetry, no_states, flux_points, False, True, i)
        my_run.prepare_operators()
        my_run.override_parameters(EC, EJ, alpha, assymetry)
        my_run.simulate([False, False])
        interaction_dict[i] = my_run.spectrum_eigvals

    with ("states.pkl", "w") as fout:
        pickle.dump(interaction_dict, fout)
